...
def mail_control(req):
    userna = req['username']
    deadline = time_control({req['method']:'regist'})
    unameDeadline = make_password(userna+deadline,None,SSH_KEY)
    activeURL = "127.0.0.1:8000/account/active/?name="+userna.decode('utf-8')+"&un="+unameDeadline+"&t="+deadline
    message = ("亲爱的"+userna+"，你好！\n感谢注册smart，请在7天内点击以下链接完成注册:\n").decode('utf-8')+activeURL
